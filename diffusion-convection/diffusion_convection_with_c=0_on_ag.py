...
a1 = - dt*inner(c*u, grad(v))*dx + D1*dt*inner(grad(c), grad(v))*dx(0) + D2*dt*inner(grad(c), grad(v))*dx(1) + dt*inner(c*dot(u,n), v)*ds(0) + dt*inner(c*dot(u,n), v)*ds(1)
a2 = - dt*inner(c*u, grad(v))*dx + D1*dt*inner(grad(c), grad(v))*dx(0) +D2*dt*inner(grad(c), grad(v))*dx(1) + dt*inner(c*dot(u,n), v)*ds(1) # + D*dt*inner(dot(grad(c),n), v)*ds(1) this adds diffusion on the AG
A = assemble(a1)
A = M_lumped + A
L = c_n*v*dx
