...
top_subdomain = Rectangle(Point(0, 0.06), Point(0.1, 0.1)) - Rectangle(Point(0.01, 0.06), Point(0.09, 0.09))
domain.set_subdomain(1, subdomain)
domain.set_subdomain(2, top_subdomain)
...
(v, q) = TestFunctions(W)
f = Constant((0.0, 0.0))
a = -mu1*inner(grad(u), grad(v))*dx(0) - mu1*inner(grad(u), grad(v))*dx(2) - mu2*inner(grad(u), grad(v))*dx(1) + div(v)*p*dx + q*div(u)*dx
L = inner(f, v)*dx
...
ufile_pvd = File("Stokes with top subdomain plots/velocity.pvd")
ufile_pvd << u
pfile_pvd = File("Stokes with top subdomain plots/pressure.pvd")
pfile_pvd << p

# Save velocity
with XDMFFile(MPI.comm_world, '../diffusion-convection/velocity_with_top_subdomain.xdmf') as xdmf:
